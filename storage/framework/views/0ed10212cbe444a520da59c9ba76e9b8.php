...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <div class="flex items-center gap-2 px-4 py-2 rounded-2xl border
        <?php echo e($totalPending === 0 ? 'bg-status-successs border-status-success/20' : 'bg-status-warnings border-status-warning/20'); ?>">
        <span class="material-symbols-outlined text-[18px] <?php echo e($totalPending === 0 ? 'text-status-success' : 'text-status-warning'); ?>">
            <?php echo e($totalPending === 0 ? 'check_circle' : 'pending_actions'); ?>

        </span>
        <span class="text-sm font-bold <?php echo e($totalPending === 0 ? 'text-status-success' : 'text-status-warning'); ?>">
            <?php echo e($totalUploaded); ?>/<?php echo e($totalRequired); ?> Uploaded
        </span>
    </div>
</div>
    ...
    <div class="flex items-center justify-between mb-3">
        <div>
            ...
            <p class="text-xs text-brand-sub mt-0.5">
                ...
                <?php else: ?>
                    <?php echo e($totalPending); ?> document<?php echo e($totalPending > 1 ? 's' : ''); ?> still required
                <?php endif; ?>
            </p>
        </div>
        <span class="text-3xl font-black <?php echo e($totalPending === 0 ? 'text-status-success' : 'text-brand-accent'); ?>">
            <?php echo e($progressPct); ?>%
        </span>
    </div>
    <div class="w-full h-3 rounded-full bg-brand-muted overflow-hidden">
        <div class="h-full rounded-full transition-all duration-700
            <?php echo e($totalPending === 0 ? 'bg-status-success' : 'bg-gradient-to-r from-brand-accent to-sky-400'); ?>"
...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
    <?php $__currentLoopData = $requiredDocs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $docType => $docLabel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php
        ...
        <div class="card flex flex-col gap-0 p-0 overflow-hidden group">
            ...
            <div class="h-1.5 w-full <?php echo e($isUploaded ? 'bg-status-success' : 'bg-status-danger'); ?>"></div>
                ...
                <div class="flex items-start justify-between gap-3 mb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0
                            <?php echo e($isUploaded ? 'bg-status-successs' : 'bg-status-dangers'); ?>">
                            <span class="material-symbols-outlined text-[22px]
                                <?php echo e($isUploaded ? 'text-status-success' : 'text-status-danger'); ?>">
                                <?php echo e($isUploaded ? 'task' : 'description'); ?>

                            </span>
                        </div>
                        <div>
                            <p class="text-sm font-bold text-brand-text leading-tight"><?php echo e($docLabel); ?></p>
                            <p class="text-xs text-brand-sub mt-0.5">
                                <?php echo e($isUploaded ? 'Uploaded' : 'Required · Not Uploaded'); ?>

                            </p>
                        </div>
                    </div>
                    <span class="badge <?php echo e($isUploaded ? 'badge-success' : 'badge-danger'); ?> flex-shrink-0">
                        <?php echo e($isUploaded ? 'Done' : 'Pending'); ?>
                ...
                <?php if($isUploaded): ?>
                    ...
                    <div class="rounded-xl bg-brand-muted border border-brand-border p-3 mb-4 flex items-center gap-3 flex-1">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0
                            <?php echo e($isPdf ? 'bg-red-100' : 'bg-blue-100'); ?>">
                            <span class="material-symbols-outlined text-[20px] <?php echo e($isPdf ? 'text-red-500' : 'text-blue-500'); ?>">
                                <?php echo e($isPdf ? 'picture_as_pdf' : 'image'); ?>

                            </span>
                        </div>
                        <div class="min-w-0">
                            <p class="text-xs font-semibold text-brand-text truncate" title="<?php echo e($doc->file_name); ?>">
                        ...
                        <a href="<?php echo e(Storage::disk('public')->url($doc->file_path)); ?>"
                           target="_blank"
                    ...
                    <div class="flex gap-2 mt-auto">
                        ...
                        <button type="button"
                                onclick="openUploadModal('<?php echo e($docType); ?>', '<?php echo e($docLabel); ?>')"
                                class="flex-1 btn-secondary text-xs py-2 justify-center">
                            <span class="material-symbols-outlined text-[15px]">upload_file</span>
                            Replace
                        </button>
                        
                        <form method="POST" action="<?php echo e(route('student.documents.delete')); ?>"
...
<div id="upload-modal"
    ...
    <div class="bg-brand-surface rounded-2xl shadow-2xl border border-brand-border w-full max-w-md transform transition-all duration-300 scale-95 opacity-0"
        ...
        <form id="upload-form"
              method="POST"
              action="<?php echo e(route('student.documents.upload')); ?>"
            ...
            <?php if($errors->hasAny(['document', 'document_type'])): ?>
                <p class="text-xs text-status-danger flex items-center gap-1">
                    <span class="material-symbols-outlined text-[14px]">error</span>
                    <?php echo e($errors->first('document') ?? $errors->first('document_type')); ?>
...
<script>
    ...
    <?php if($errors->has('document') || $errors->has('document_type')): ?>
        <?php
            $oldType  = old('document_type', '');
            $oldLabel = $requiredDocs[$oldType] ?? $oldType;
        ?>
        document.addEventListener('DOMContentLoaded', () => {
            openUploadModal('<?php echo e($oldType); ?>', '<?php echo e($oldLabel); ?>');
        });
    <?php endif; ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ramaw\Desktop\CollegeProject\IntelliCampus ERP\resources\views/student/documents/index.blade.php ENDPATH**/ ?>