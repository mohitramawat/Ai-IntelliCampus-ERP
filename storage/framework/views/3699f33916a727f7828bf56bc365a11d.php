    ...
    <div class="relative bg-gradient-to-r from-brand-accent to-sky-600 rounded-2xl p-6 sm:p-8 mb-8 overflow-hidden shadow-accent">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="text-right hidden sm:block">
                ...
                <p class="text-3xl font-extrabold text-white">₹<?php echo e(number_format($fees->sum('unit_fee') - $fees->sum('total_paid'), 0)); ?></p>
            </div>
        </div>
    </div>
        ...
        <div class="card p-0 overflow-hidden">
            ...
            <div class="p-6 bg-brand-muted border-b border-brand-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex items-center gap-4">
                    <div class="kpi-icon <?php echo e($fee->status === 'paid' ? 'bg-status-successs' : 'bg-brand-acents'); ?>">
                        <span class="material-symbols-outlined <?php echo e($fee->status === 'paid' ? 'text-status-success' : 'text-brand-accent'); ?> text-[22px]">
                            <?php echo e($fee->status === 'paid' ? 'verified' : 'receipt_long'); ?>

                        </span>
                    </div>
                    <div>
                        <div class="flex items-center gap-3">
                            <h3 class="text-lg font-bold text-brand-text"><?php echo e($fee->unit_name); ?></h3>
                        ...
                        <p class="text-sm text-brand-sub mt-0.5">Total: ₹<?php echo e(number_format($fee->unit_fee, 2)); ?></p>
                    </div>
                </div>
                ...
                <div class="w-full sm:w-64">
                    <div class="flex justify-between text-xs font-semibold mb-1.5">
                        <span class="text-status-success">Paid: ₹<?php echo e(number_format($fee->total_paid, 0)); ?></span>
                        <span class="text-status-warning">Due: ₹<?php echo e(number_format($balance, 0)); ?></span>
                    </div>
                    <div class="w-full h-2.5 bg-brand-border rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-brand-accent to-status-success rounded-full transition-all duration-700" style="width: <?php echo e($progress); ?>%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <button @click="openPayment('<?php echo e($fee->id); ?>', '<?php echo e($fee->unit_name); ?> — <?php echo e($nextInstallment->installment_name ?? "Installment"); ?>', <?php echo e($nextDueAmount); ?>)" 
                        ...
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <span class="text-xs font-semibold text-brand-sub">Due <?php echo e($nextInstallment->due_date->format('d M')); ?></span>
                        </div>
                        <h5 class="text-sm font-bold text-brand-text mb-1">Pay Next Installment</h5>
                        <p class="text-xl font-extrabold text-brand-accent">₹<?php echo e(number_format($nextDueAmount, 2)); ?></p>
                    </button>
                    <?php endif; ?>

                    
                    <button @click="openPayment('<?php echo e($fee->id); ?>', '<?php echo e($fee->unit_name); ?> — Full Clearance', <?php echo e($balance); ?>)" 
                        ...
                        <p class="text-xl font-extrabold text-status-success">₹<?php echo e(number_format($balance, 2)); ?></p>
                    </button>
                    ...
                    <div class="p-5 rounded-2xl border border-brand-border bg-brand-surface">
                        ...
                        <div class="flex gap-2">
                            <div class="relative flex-1">
                                ...
                                <input type="number" x-model="customAmount['<?php echo e($fee->id); ?>']" min="1" max="<?php echo e($balance); ?>" placeholder="Enter amount" class="input pl-7 text-sm">
                            </div>
                            <button @click="if(customAmount['<?php echo e($fee->id); ?>'] > 0) openPayment('<?php echo e($fee->id); ?>', '<?php echo e($fee->unit_name); ?> — Partial', customAmount['<?php echo e($fee->id); ?>'])" 
                ...
                <div class="mt-6 pt-6 border-t border-brand-border">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <?php $__currentLoopData = $fee->installments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ins): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="p-3 rounded-xl bg-brand-muted border border-brand-border">
                            <p class="text-xs font-bold text-brand-text mb-0.5"><?php echo e($ins->installment_name ?? 'Installment ' . $ins->installment_number); ?></p>
                            <p class="text-[10px] text-brand-sub mb-2">Due: <?php echo e($ins->due_date->format('d M Y')); ?></p>
                            <div class="flex justify-between items-center">
                                <span class="text-sm font-bold <?php echo e($ins->status === 'paid' ? 'text-status-success' : 'text-brand-text'); ?>">₹<?php echo e(number_format($ins->installment_amount, 0)); ?></span>
                                <?php if($ins->status === 'paid'): ?>
                                    <span class="material-symbols-outlined text-status-success text-[16px]">check_circle</span>
                                <?php elseif($ins->paid_amount > 0): ?>
                                    <span class="badge badge-accent text-[9px]"><?php echo e(round(($ins->paid_amount/$ins->installment_amount)*100)); ?>%</span>
                                <?php endif; ?>
                            </div>
                        </div>
...
function paymentManager() {
    return {
        ...
        async processPayment() {
            ...
            try {
                const res = await fetch('<?php echo e(route("student.fees.pay.simulate")); ?>', {
                    method: 'POST',
                    headers: {
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ramaw\Desktop\CollegeProject\IntelliCampus ERP\resources\views/student/fees/index.blade.php ENDPATH**/ ?>