...
<div style="display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:22px">
    <div>
        <div style="font-size:12px;color:#64748B;margin-bottom:4px;display:flex;gap:5px;align-items:center">
            ...
            <span style="color:#0EA5E9;font-weight:700">Subjects</span>
        </div>
        <h1 style="font-size:26px;font-weight:800;color:#1A202C;margin:0">Subjects</h1>
        <p style="font-size:13px;color:#64748B;margin:2px 0 0">Manage subjects per course and semester.</p>
    </div>
    <button onclick="openAdd()" class="btn-primary" style="display:flex;align-items:center;gap:6px">
        <span class="material-symbols-outlined" style="font-size:18px">add</span> Add Subject
    </button>
</div>


<div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:16px">
    <select id="filter-course" class="f-input" style="width:auto;min-width:220px" onchange="updateSemFilter(); if(dt) dt.ajax.reload()">
        <option value="">All Courses</option>
        <?php $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <option value="<?php echo e($c->id); ?>" data-units="<?php echo e($c->total_units); ?>"><?php echo e($c->code); ?> — <?php echo e($c->name); ?></option>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </select>
    <select id="filter-sem" class="f-input" style="width:auto;min-width:140px" onchange="if(dt) dt.ajax.reload()">
        <option value="">All Semesters</option>
        <?php $__currentLoopData = range(1,12); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<div class="card">
    <div style="overflow-x:auto">
        <table id="subjects-table" class="w-full" style="width:100%">
            <thead>
                <tr style="border-bottom:2px solid #F0F4F8">
                    <th class="dt-th">#</th>
                    <th class="dt-th">Subject Name</th>
...
<div id="modal" style="display:none" class="md-overlay" onclick="if(event.target===this)closeModal()">
    <div class="md-box">
        <div class="md-head">
            <h3 class="md-title" id="modal-title">Add Subject</h3>
        ...
        <div class="md-body">
            <form onsubmit="submitForm(event)">
                ...
                <div style="margin-bottom:14px">
                    ...
                    <select id="f-course" class="f-input" onchange="updateModalSems()">
                ...
                <div class="md-grid" style="margin-bottom:14px">
                    <div class="f-group">
                        ...
                        <select id="f-sem" class="f-input">
                            <option value="">— Select Course First —</option>
                        </select>
                        <p class="f-error" id="err-semester"></p>
                    </div>
                    <div class="f-group">
                        <label class="f-label">Subject Code *</label>
                        <input type="text" id="f-code" class="f-input" placeholder="e.g. MCA101" style="text-transform:uppercase">
                        <p class="f-error" id="err-code"></p>
                    </div>
                </div>
                <div style="margin-bottom:14px">
                    <label class="f-label">Subject Name *</label>
                    <input type="text" id="f-name" class="f-input" placeholder="e.g. Data Structures">
...
<div id="del-modal" style="display:none" class="md-overlay" onclick="if(event.target===this)closeDelModal()">
    <div class="md-box" style="max-width:380px">
        ...
        <div class="md-body">
            <p style="font-size:13px;color:#64748B;margin:0 0 20px">Delete subject <strong id="del-name"></strong>?</p>
            <div style="display:flex;gap:10px;justify-content:flex-end">
...
dt = $('#subjects-table').DataTable({
    processing: true, serverSide: true,
    ajax: {
        url: '<?php echo e(route("writer.master.subjects.datatable")); ?>',
        data: d => {
            d.course_id = $('#filter-course').val();
            d.semester  = $('#filter-sem').val();
        }
    },
    columns: [
        ...
        { data: 'sem_label',   name: 'semester',     width: '100px' },
    ...
    pageLength: 20,
    language: { search: '', searchPlaceholder: 'Search subjects...', processing: '<span style="color:#0EA5E9">Loading...</span>' },
    dom: '<"dt-toolbar"f>t<"dt-bottombar"ip>',
});

// Populate semester dropdown from selected course's total_units
function populateSems(selEl, val = '') {
    const units = parseInt(selEl.options[selEl.selectedIndex]?.dataset.units) || 12;
...
function updateModalSems() { populateSems(document.getElementById('f-course')); }
function updateSemFilter()  {
    // Don't change filter-sem options based on course filter, just reload
}
...
function openAdd() {
    ...
    document.getElementById('f-sem').innerHTML = '<option value="">— Select Course First —</option>';
    clearErrors();
    document.getElementById('modal').style.display = 'flex';
}

async function editSubject(id) {
    const res = await fetch(`<?php echo e(url('writer/master/subjects')); ?>/${id}`);
    ...
    populateSems(document.getElementById('f-course'), d.semester);
...
async function submitForm(e) {
    ...
    const url    = id ? `<?php echo e(url('writer/master/subjects')); ?>/${id}` : '<?php echo e(route("writer.master.subjects.store")); ?>';
...
function deleteSubject(id, name) { deleteId=id; document.getElementById('del-name').textContent=name; document.getElementById('del-modal').style.display='flex'; }
function closeDelModal() { document.getElementById('del-modal').style.display='none'; deleteId=null; }
async function confirmDelete() {
    const res = await fetch(`<?php echo e(url('writer/master/subjects')); ?>/${deleteId}`, { method:'DELETE', headers:{'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'} });
    const data = await res.json();
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ramaw\Desktop\CollegeProject\IntelliCampus ERP\resources\views\writer\master\subjects\index.blade.php ENDPATH**/ ?>