...
<div x-data="teacherAttendance()" @timer-expired.window="autoCloseSession()">
    ...
    <div class="ta-card ta-fade" style="--d:.12s"
         ...
         x-transition
         style="display:<?php echo e($activeSession ? 'none' : 'block'); ?>">
            ...
            <div class="ta-field">
                ...
                <select x-model="formData.period_number" class="ta-select">
                    <?php $__currentLoopData = range(1, 8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($p); ?>">Period <?php echo e($p); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
        </div>
    ...
    <div x-show="sessionState === 'active'" x-cloak class="ta-active-grid ta-fade" style="--d:.15s; display:<?php echo e($activeSession ? 'grid' : 'none'); ?>">
        ...
        <div class="ta-card">
            ...
            <div style="position:relative;z-index:1">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px">
                    ...
                    <div class="ta-detail-chip"
                         x-data="timer(<?php echo e($activeSession ? \Carbon\Carbon::parse($activeSession->start_time)->addMinutes($activeSession->attendance_window_minutes)->timestamp : 0); ?>)">
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('teacherAttendance', () => ({
        sessionState: '<?php echo e($activeSession ? "active" : "config"); ?>',
        loading: false,

        // Raw data from server
        allBatches: <?php echo json_encode($batches, 15, 512) ?>,
        allSubjects: <?php echo json_encode($subjects, 15, 512) ?>,
        ...
        activeData: {
            id: '<?php echo e($activeSession?->id ?? ""); ?>',
            subject_name: '<?php echo e($activeSession?->subject?->name ?? ""); ?>',
            batch_name: '<?php echo e($activeSession?->batch?->name ?? ""); ?>',
            period: '<?php echo e($activeSession?->period_number ?? ""); ?>',
            start_time: '<?php echo e($activeSession ? \Carbon\Carbon::parse($activeSession->start_time)->format("H:i") : ""); ?>',
            present: <?php echo e($activeSession?->present_count ?? 0); ?>,
            total: <?php echo e($activeSession?->total_students ?? 0); ?>
        ...
        async pollStudents() {
            ...
            try {
                const res = await fetch(`<?php echo e(url('teacher/attendance/session')); ?>/${this.activeData.id}/students`);
        ...
        async submitStart() {
            try {
                const res = await fetch('<?php echo e(route("teacher.attendance.start")); ?>', {
                    method: 'POST',
        ...
        async closeSession() {
            this.loading = true;
            try {
                const res = await fetch('<?php echo e(route("teacher.attendance.close")); ?>', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
        ...
        async openAiCamera() {
            ...
            if (!this.enrolledStudents.length) {
                try {
                    const res = await fetch(`<?php echo e(url('teacher/attendance/session')); ?>/${this.activeData.id}/biometrics`);
                    const data = await res.json();
        ...
        async submitAiBulkAttendance() {
            ...
            try {
                const res = await fetch(`<?php echo e(url('teacher/attendance/session')); ?>/${this.activeData.id}/mark-bulk`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
        ...
        async submitManualAttendance(studentId) {
            try {
                const res = await fetch(`<?php echo e(url('teacher/attendance/session')); ?>/${this.activeData.id}/mark-manual`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ramaw\Desktop\CollegeProject\IntelliCampus ERP\resources\views/teacher/attendance/index.blade.php ENDPATH**/ ?>