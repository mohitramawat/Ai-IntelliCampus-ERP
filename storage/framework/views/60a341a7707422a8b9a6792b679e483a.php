...
<div x-data="ultrasonicTeacher()">
    ...
    <div class="us-card us-fade" style="--d: 0.1s" x-show="!isActive" x-cloak style="display: <?php echo e($activeSession ? 'none' : 'block'); ?>">
    ...
    <div class="us-card us-fade" style="--d: 0.15s; text-align: center; padding: 60px 32px;" x-show="isActive" x-cloak style="display: <?php echo e($activeSession ? 'block' : 'none'); ?>">
        ...
        <div class="us-token-box">
            ...
            <span class="us-token-value"><?php echo e($activeSession ? $activeSession->ultrasonic_token : '----'); ?></span>
        </div>
        ...
        <div style="display: flex; gap: 40px; justify-content: center; margin-bottom: 40px;">
            <div style="text-align: center;">
                ...
                <p style="font-size: 24px; font-weight: 900; color: var(--us-emerald);" x-text="presentCount"><?php echo e($activeSession ? $activeSession->present_count : 0); ?></p>
            </div>
            <div style="text-align: center;">
                <p style="font-size: 11px; font-weight: 700; color: var(--us-sub); text-transform: uppercase;">Total Class Size</p>
                <p style="font-size: 24px; font-weight: 900; color: var(--us-text);"><?php echo e($activeSession ? $activeSession->total_students : 0); ?></p>
            </div>
        </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('ultrasonicTeacher', () => ({
        isActive: <?php echo e($activeSession ? 'true' : 'false'); ?>,
        loading: false,
        presentCount: <?php echo e($activeSession ? $activeSession->present_count : 0); ?>,
        ...
        init() {
            <?php if($activeSession): ?>
                this.initAudio("<?php echo e($activeSession->ultrasonic_token); ?>");
        ...
        async pollStudents() {
            try {
                const res = await fetch('<?php echo e(url("teacher/attendance/session")); ?>/<?php echo e($activeSession->id ?? 0); ?>/students');
        ...
        async startBroadcast() {
            this.loading = true;
            try {
                const res = await fetch('<?php echo e(route("teacher.ultrasonic.start")); ?>', {
                    method: 'POST',
        ...
        async stopBroadcast() {
            ...
            const res = await fetch('<?php echo e(route("teacher.attendance.close")); ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
                body: JSON.stringify({ lecture_session_id: <?php echo e($activeSession->id ?? 0); ?> })
            });
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ramaw\Desktop\CollegeProject\IntelliCampus ERP\resources\views/teacher/ultrasonic/index.blade.php ENDPATH**/ ?>