...
const dtDepts = $('#dept-table').DataTable({
    ...
    ajax: '{{ route("admin.master.departments.datatable") }}',
    columns: [
...
const dtCourses = $('#course-table').DataTable({
    processing: true, serverSide: true,
    ajax: { url: '{{ route("admin.master.courses.datatable") }}', data: d => { d.department_id = $('#dept-filter').val(); } },
    columns: [
        { data: 'DT_RowIndex', orderable: false, searchable: false, width: '50px' },
        { data: 'name',          name: 'name' },
        { data: 'code',          name: 'code', width: '80px' },
...
const dtBatches = $('#batch-table').DataTable({
    processing: true, serverSide: true,
    ajax: '{{ route("admin.master.batches.datatable") }}',
    columns: [
        { data: 'DT_RowIndex',  orderable: false, searchable: false, width: '50px' },
        { data: 'name',         name: 'name' },
...
const dtSubjects = $('#subject-table').DataTable({
    processing: true, serverSide: true,
    ajax: '{{ route("admin.master.subjects.datatable") }}',
    columns: [
        { data: 'DT_RowIndex', orderable: false, searchable: false, width: '50px' },
        { data: 'name',        name: 'name' },
...
const delUrls = {
    department: '{{ url("admin/master/departments") }}',
    course:     '{{ url("admin/master/courses") }}',
    batch:      '{{ url("admin/master/batches") }}',
    subject:    '{{ url("admin/master/subjects") }}',
};
const dtMap = { department: dtDepts, course: dtCourses, batch: dtBatches, subject: dtSubjects };

async function doDelete() {
    const res  = await fetch(`${delUrls[delType]}/${delId}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } });
