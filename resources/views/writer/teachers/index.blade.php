            ...
            <form onsubmit="submitForm(event)">
                ...
                <div class="md-grid" style="margin-bottom:14px">
                    ...
                    <div>
                        ...
                        <input type="date" id="f-joining" class="f-input" value="{{ date('Y-m-d') }}">
...
dt = $('#teachers-table').DataTable({
    ...
    ajax: {
        url: '{{ route("writer.teachers.datatable") }}',
...
async function editTeacher(id) {
    const res = await fetch(`{{ url('writer/teachers') }}/${id}`, { headers: { 'Accept': 'application/json' } });
...
async function submitForm(e) {
    ...
    const url = id ? `{{ url('writer/teachers') }}/${id}` : '{{ route("writer.teachers.store") }}';
...
async function confirmDelete() {
    const res = await fetch(`{{ url('writer/teachers') }}/${deleteId}`, { 
        method:'DELETE', 
...
async function toggleHod(id, name, isHod) {
    ...
    const res = await fetch(`{{ url('writer/teachers') }}/${id}/toggle-hod`, {
        method: 'POST',
        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' }
    });
    const data = await res.json();
