...
<div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:16px">
    <select id="filter-course" class="f-input" style="width:auto;min-width:220px" onchange="updateSemFilter(); if(dt) dt.ajax.reload()">
        ...
        @foreach($courses as $c)
            <option value="{{ $c->id }}" data-units="{{ $c->total_units }}">{{ $c->code }} — {{ $c->name }}</option>
        @endforeach
    </select>
...
dt = $('#subjects-table').DataTable({
    ...
    ajax: {
        url: '{{ route("writer.master.subjects.datatable") }}',
        data: d => {
...
async function editSubject(id) {
    const res = await fetch(`{{ url('writer/master/subjects') }}/${id}`);
...
async function submitForm(e) {
    ...
    const url    = id ? `{{ url('writer/master/subjects') }}/${id}` : '{{ route("writer.master.subjects.store") }}';
...
async function confirmDelete() {
    const res = await fetch(`{{ url('writer/master/subjects') }}/${deleteId}`, { method:'DELETE', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'} });
    const data = await res.json();
