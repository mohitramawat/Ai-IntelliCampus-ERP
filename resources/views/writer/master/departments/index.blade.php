...
dt = $('#depts-table').DataTable({
    ...
    ajax: '{{ route("writer.master.departments.datatable") }}',
    columns: [
...
async function editDept(id) {
    const res = await fetch(`{{ url('writer/master/departments') }}/${id}`);
...
async function submitForm(e) {
    ...
    try {
        ...
        const url    = id ? `{{ url('writer/master/departments') }}/${id}` : '{{ route("writer.master.departments.store") }}';
...
async function confirmDelete() {
    const res  = await fetch(`{{ url('writer/master/departments') }}/${deleteId}`, {
