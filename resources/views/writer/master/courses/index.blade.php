...
dt = $('#courses-table').DataTable({
    ...
    ajax: {
        url: '{{ route("writer.master.courses.datatable") }}',
...
async function editCourse(id) {
    const res = await fetch(`{{ url('writer/master/courses') }}/${id}`);
...
async function submitForm(e) {
    ...
    const url    = id ? `{{ url('writer/master/courses') }}/${id}` : '{{ route("writer.master.courses.store") }}';
...
async function confirmDelete() {
    const res = await fetch(`{{ url('writer/master/courses') }}/${deleteId}`, { method:'DELETE', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'} });
    const data = await res.json();
