...
dt = $('#batches-table').DataTable({
    ...
    ajax: {
        url: '{{ route("writer.master.batches.datatable") }}',
        data: d => {
...
async function editBatch(id) {
    const res = await fetch(`{{ url('writer/master/batches') }}/${id}`);
...
async function submitForm(e) {
    ...
    const url    = id ? `{{ url('writer/master/batches') }}/${id}` : '{{ route("writer.master.batches.store") }}';
...
async function confirmDelete() {
    const res = await fetch(`{{ url('writer/master/batches') }}/${deleteId}`, { method:'DELETE', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'} });
    const data = await res.json();
