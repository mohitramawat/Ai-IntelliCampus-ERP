...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <a href="{{ route('writer.students.create') }}" class="btn-primary self-start">
...
<div class="mb-5 p-4 rounded-xl bg-status-successs border border-status-success/20 flex items-center gap-3">
    ...
    <p class="text-sm font-bold text-status-success">{{ session('success') }}</p>
</div>
@endif
...
const table = $('#students-table').DataTable({
    ...
    ajax: {
        url: '{{ route('writer.students.datatable') }}',
        data: d => {
