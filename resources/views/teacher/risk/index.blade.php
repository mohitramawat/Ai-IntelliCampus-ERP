...
{{-- ─────────────────────────────────────────────────────────────────────────────
     AI ATTENDANCE RISK PREDICTION — PREMIUM TEACHER DASHBOARD
     ───────────────────────────────────────────────────────────────────────────── --}}
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    ...
    <div class="relative rounded-2xl bg-white border border-slate-100 p-5 transition-all duration-300 hover:-translate-y-1.5 shadow-[0_4px_12px_rgba(0,0,0,0.02)] {{ $kpi['glow'] }}">
        ...
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl {{ $kpi['bg'] }} flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
...
<div class="rounded-2xl bg-white border border-slate-100 p-5 mb-8 shadow-[0_4px_12px_rgba(0,0,0,0.02)]">
    <div class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
        ...
        <form method="GET" action="{{ route('teacher.risk.index') }}" class="flex gap-2 flex-1 max-w-md">
        ...
        <div class="flex items-center gap-1.5 p-1 bg-slate-50 border border-slate-100 rounded-xl overflow-x-auto max-w-full">
            @foreach([
                ...
                <a href="{{ route('teacher.risk.index', ['risk' => $val, 'search' => $search]) }}"
...
{{-- Dynamic Neural Process Loader (Hidden initially) --}}
...
<div id="students-grid" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
    ...
    <div class="student-card relative rounded-2xl bg-white border border-slate-100 p-6 transition-all duration-300 shadow-[0_4px_12px_rgba(0,0,0,0.01)] {{ $glowMap[$riskLvl] }}"
         data-student-id="{{ $student->id }}"
         data-student-name="{{ $student->user?->name }}"
         id="card-{{ $student->id }}">

        {{-- Accent Colored Border Top --}}
        <div class="absolute left-6 right-6 top-0 h-[3px] rounded-b-md
            {{ match($riskLvl) { 'high' => 'bg-red-400', 'medium' => 'bg-yellow-400', 'low' => 'bg-emerald-400', default => 'bg-slate-200' } }}" id="topbar-{{ $student->id }}"></div>
        ...
        <div class="flex items-start justify-between mb-5 mt-1">
            <div class="flex items-center gap-3 min-w-0">
                <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 font-extrabold text-sm
                    {{ match($riskLvl) { 'high' => 'bg-red-50 text-red-500', 'medium' => 'bg-yellow-50 text-yellow-500', 'low' => 'bg-emerald-50 text-emerald-500', default => 'bg-slate-50 text-slate-500' } }}" id="avatar-{{ $student->id }}">
                    {{ strtoupper(substr($student->user?->name ?? 'S', 0, 2)) }}
                </div>
                <div class="min-w-0">
                    <p class="text-sm font-extrabold text-slate-800 truncate leading-snug">{{ $student->user?->name ?? '—' }}</p>
                    <p class="text-[11px] font-semibold text-slate-400 truncate mt-0.5">{{ $student->batch?->course?->code ?? '' }} · Sem {{ $student->current_unit }}</p>
                </div>
            </div>
            <span class="risk-badge px-2.5 py-1 rounded-lg text-[10px] font-bold border {{ $badgeMap[$riskLvl] }} flex-shrink-0"
                  id="badge-{{ $student->id }}">
                {{ $labelMap[$riskLvl] }}
            </span>
        </div>
        ...
        <div class="mb-5 bg-slate-50/50 border border-slate-100 rounded-xl p-3.5">
            <div class="flex justify-between items-center mb-1.5">
                ...
                <span class="text-xs font-black text-slate-700 att-pct-label" id="pct-{{ $student->id }}">{{ $pct }}%</span>
            </div>
            <div class="w-full bg-slate-200/60 rounded-full h-2 overflow-hidden">
                <div class="h-2 rounded-full att-bar transition-all duration-700 {{ $barColor }}"
                     id="bar-{{ $student->id }}"
        ...
        {{-- AI Remark Panel with elegant Left Accent bar --}}
        <div class="mb-5 ai-remark-section" id="remark-{{ $student->id }}">
            @if($pred)
            <div class="p-3.5 rounded-xl bg-slate-50 border-l-4 border-slate-400">
                ...
                <p class="text-xs text-slate-600 leading-relaxed font-medium">{{ $pred->ai_remark }}</p>
            </div>
            @else
        ...
        <div class="mb-5 {{ ($pred && $pred->suggested_action) ? '' : 'hidden' }}" id="action-{{ $student->id }}">
            @if($pred && $pred->suggested_action)
            <div class="p-3.5 rounded-xl border
                {{ match($riskLvl) { 'high' => 'bg-red-50/30 border-red-100', 'medium' => 'bg-yellow-50/30 border-yellow-100', default => 'bg-emerald-50/30 border-emerald-100' } }}">
                <p class="text-[10px] font-bold uppercase tracking-wider mb-1
                    {{ match($riskLvl) { 'high' => 'text-red-500', 'medium' => 'text-yellow-500', default => 'text-emerald-500' } }}">
                    Prescribed Intervention
                </p>
                <p class="text-xs text-slate-600 leading-relaxed font-semibold">{{ $pred->suggested_action }}</p>
            </div>
            @endif
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-100">
            ...
            <button class="btn-predict-one flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-bold transition-all
                    ...
                    data-student-id="{{ $student->id }}"
                    data-predict-url="{{ route('teacher.risk.predict', $student->id) }}">
