...
<div class="us-card us-fade" style="--d: 0.1s; text-align: center; min-height: 400px; display: flex; flex-direction: column; align-items: center; justify-content: center;">
    ...
    @elseif($activeSession->is_marked)
        ...
        <p style="font-size: 15px; font-weight: 600; color: var(--us-sub); margin: 0;">Your presence for <strong>{{ $activeSession->subject->code }}</strong> has been securely verified.</p>
...
@push('scripts')
@if($activeSession && !$activeSession->is_marked)
<script>
let audioCtx;
let analyser;
let dataArray;
...
async function submitAttendance(token) {
    ...
    try {
        const res = await fetch('{{ route("student.ultrasonic.mark") }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            body: JSON.stringify({
                lecture_session_id: {{ $activeSession->id }},
