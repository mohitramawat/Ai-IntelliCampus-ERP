...
<div class="relative rounded-2xl p-6 mb-6 overflow-hidden shadow-[0_4px_14px_rgba(139,92,246,0.35)]" style="background: linear-gradient(to right, #8B5CF6, #A855F7);">
    ...
    <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            ...
            <p class="text-white/70 text-sm mt-1">{{ now()->format('l, d F Y') }} · {{ $dept?->name ?? 'Department' }} Control</p>
        </div>
        @if($dept)
        <div style="background:rgba(255,255,255,0.15);backdrop-filter:blur(4px);border:1px solid rgba(255,255,255,0.25);border-radius:12px;padding:10px 18px;text-align:center">
            ...
            <p style="font-size:16px;color:#fff;font-weight:800;margin:2px 0 0">{{ $dept->code ?? $dept->name }}</p>
        </div>
        @endif
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    ...
    <div class="card">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('hod.teachers.index') }}" class="text-xs font-semibold text-brand-accent hover:underline">View All</a>
        </div>
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-brand-muted transition-colors">
                    <div class="w-8 h-8 rounded-full bg-status-successs flex items-center justify-center flex-shrink-0">
                        <span class="text-xs font-bold text-status-success">{{ strtoupper(substr($t->user?->name ?? 'T', 0, 1)) }}</span>
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-brand-text truncate">{{ $t->user?->name ?? '—' }}</p>
                        <p class="text-[11px] text-brand-sub truncate">{{ $t->user?->email ?? '' }}</p>
                    </div>
                </div>
                @endforeach
                @if($teachers->count() > 5)
                <p class="text-xs text-brand-sub text-center pt-1">+ {{ $teachers->count() - 5 }} more teachers</p>
                @endif
            </div>
        @endif
    </div>
    ...
    <div class="card">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('hod.students.index') }}" class="text-xs font-semibold text-brand-accent hover:underline">View All</a>
        </div>
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-brand-muted transition-colors">
                    <div class="w-8 h-8 rounded-full bg-brand-acents flex items-center justify-center flex-shrink-0">
                        <span class="text-xs font-bold text-brand-accent">{{ strtoupper(substr($s->user?->name ?? 'S', 0, 1)) }}</span>
                    </div>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-brand-text truncate">{{ $s->user?->name ?? '—' }}</p>
                        <p class="text-[11px] text-brand-sub truncate">
                            {{ $s->batch?->course?->code ?? '' }} · {{ $s->batch?->name ?? '' }}
                        </p>
                    </div>
                    <span class="text-[10px] text-brand-sub flex-shrink-0 ml-auto">{{ $s->created_at->diffForHumans() }}</span>
                </div>
                @endforeach
            </div>
        @endif
...
<div class="card mt-5">
    ...
    @if(isset($recentOverrides) && $recentOverrides->isEmpty())
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-brand-border/50">
                    @foreach($recentOverrides as $record)
                        ...
                        <td class="py-2.5 px-3">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-status-warnings flex items-center justify-center flex-shrink-0">
                                    <span class="text-[10px] font-bold text-status-warning">{{ strtoupper(substr($record->student->user->name ?? 'S', 0, 1)) }}</span>
                                </div>
                                <span class="text-sm font-semibold text-brand-text">{{ $record->student->user->name ?? '—' }}</span>
                            </div>
                        </td>
                        <td class="py-2.5 px-3 text-xs font-medium text-brand-sub">{{ $record->lectureSession->subject->name ?? '—' }}</td>
                        <td class="py-2.5 px-3 text-xs text-brand-sub">{{ $record->lectureSession->teacher->user->name ?? '—' }}</td>
                        <td class="py-2.5 px-3 text-xs text-brand-sub">{{ \Carbon\Carbon::parse($record->marked_at)->format('d M h:i A') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
