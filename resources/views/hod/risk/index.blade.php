...
{{-- ─────────────────────────────────────────────────────────────────────────────
     AI ATTENDANCE RISK PREDICTION — PREMIUM HOD DASHBOARD
     ───────────────────────────────────────────────────────────────────────────── --}}

{{-- Hero Banner with Flawless Gradient & Premium Glass Counters --}}
<div class="relative rounded-2xl p-8 mb-6 overflow-hidden shadow-[0_4px_20px_rgba(124,58,237,0.3)] border border-indigo-500/10" style="background: linear-gradient(135deg, #7c3aed 0%, #4f46e5 50%, #6d28d9 100%);">
    {{-- Elegant Glass Circular Overlays --}}
    ...
    <div class="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div class="flex gap-4">
            <div class="px-5 py-3.5 rounded-xl text-center bg-white/10 backdrop-blur-md border border-white/20 shadow-sm min-w-[110px]">
                ...
                <p class="text-2xl font-black text-white leading-none mb-0.5">{{ $summary['analysed'] }}/{{ $summary['total_students'] }}</p>
            ...
            <div class="px-5 py-3.5 rounded-xl text-center bg-white/10 backdrop-blur-md border border-white/20 shadow-sm min-w-[110px]">
                ...
                <p class="text-2xl font-black text-white leading-none mb-0.5">{{ number_format($summary['avg_attendance'], 1) }}%</p>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    ...
    @endphp
    @foreach($kpis as $kpi)
    <div class="relative rounded-2xl bg-white border border-slate-100 p-5 transition-all duration-300 hover:-translate-y-1.5 shadow-[0_4px_12px_rgba(0,0,0,0.02)] {{ $kpi['glow'] }}">
        {{-- Left Accent Strip --}}
        <div class="absolute left-0 top-4 bottom-4 w-[4px] rounded-r-md {{ str_replace('text', 'bg', $kpi['icon_color']) }}"></div>
        
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl {{ $kpi['bg'] }} flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined {{ $kpi['icon_color'] }} text-[24px]">{{ $kpi['icon'] }}</span>
            </div>
            <div>
                <div class="text-2xl font-black text-slate-800 tracking-tight leading-none mb-1">{{ $kpi['value'] }}</div>
                <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ $kpi['label'] }}</div>
            </div>
        </div>
    </div>
    @endforeach
        ...
        <div class="relative w-36 h-36 mb-6">
            <svg viewBox="0 0 36 36" class="w-full h-full -rotate-90">
                ...
                <circle cx="18" cy="18" r="15.9155" fill="none"
                        ...
                        stroke-dasharray="{{ $lowPct }} {{ 100 - $lowPct }}"
                        stroke-dashoffset="0"/>
                <circle cx="18" cy="18" r="15.9155" fill="none"
                        stroke="#f59e0b" stroke-width="4"
                        stroke-dasharray="{{ $medPct }} {{ 100 - $medPct }}"
                        stroke-dashoffset="{{ 100 - $lowPct }}"/>
                <circle cx="18" cy="18" r="15.9155" fill="none"
                        stroke="#ef4444" stroke-width="4"
                        stroke-dasharray="{{ $highPct }} {{ 100 - $highPct }}"
                        stroke-dashoffset="{{ 100 - $lowPct - $medPct }}"/>
            </svg>
            <div class="absolute inset-0 flex flex-col items-center justify-center leading-none">
                <span class="text-2xl font-black text-slate-800">{{ $summary['analysed'] }}</span>
        ...
        <div class="space-y-3 w-full border-t border-slate-50 pt-5">
            ...
            <div class="flex items-center justify-between text-xs font-semibold text-slate-600">
                ...
                <div class="flex items-center gap-1">
                    <span class="text-slate-800 font-extrabold">{{ $count }}</span>
        ...
        <div class="space-y-4 pt-1">
            ...
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    <span class="text-xs font-bold text-slate-500">{{ $range['label'] }}</span>
                    <span class="text-xs font-black text-slate-700">{{ $range['count'] }} Students</span>
                </div>
                <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                    <div class="{{ $range['color'] }} h-2 rounded-full transition-all duration-700 animate-pulse-slow"
                         style="width:{{ round(($range['count'] / max(1, $students->count())) * 100) }}%"></div>
                </div>
            </div>
            @endforeach
        </div>
...
<div class="rounded-2xl bg-white border border-slate-100 p-5 mb-8 shadow-[0_4px_12px_rgba(0,0,0,0.02)]">
    <div class="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
        <form method="GET" action="{{ route('hod.risk.index') }}" class="flex gap-2 flex-1 max-w-md">
        ...
        <div class="flex items-center gap-1.5 p-1 bg-slate-50 border border-slate-100 rounded-xl overflow-x-auto max-w-full">
            ...
            <a href="{{ route('hod.risk.index', ['risk' => $val, 'search' => $search]) }}"
...
<div class="rounded-2xl bg-white border border-slate-100 flex flex-col items-center justify-center py-20 text-center shadow-[0_4px_12px_rgba(0,0,0,0.02)]">
    ...
    <a href="{{ route('hod.risk.index') }}" class="text-xs font-bold text-purple-600 mt-2 hover:underline">Clear current filters</a>
</div>
@else
<div class="rounded-2xl bg-white border border-slate-100 overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.02)]">
    <div class="p-5 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
        <h3 class="text-sm font-extrabold text-slate-800 uppercase tracking-wider">
            ...
            <span class="ml-2 text-xs font-bold text-slate-400">({{ $students->count() }} Studs)</span>
        </h3>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-50" id="hod-table-body">
                ...
                @endphp
                <tr class="hover:bg-slate-50/50 transition-colors" id="row-{{ $student->id }}">
                    {{-- Student Identification --}}
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black flex-shrink-0
                                {{ match($riskLvl) { 'high' => 'bg-red-50 text-red-500', 'medium' => 'bg-yellow-50 text-yellow-500', 'low' => 'bg-emerald-50 text-emerald-500', default => 'bg-slate-50 text-slate-500' } }}" id="row-avatar-{{ $student->id }}">
                                {{ strtoupper(substr($student->user?->name ?? 'S', 0, 2)) }}
                            </div>
                            <div class="min-w-0">
                                <p class="text-sm font-bold text-slate-800 truncate leading-none mb-1.5">{{ $student->user?->name ?? '—' }}</p>
                                <p class="text-[10px] font-semibold text-slate-400 truncate">{{ $student->enrollment_number ?? '' }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    {{-- Course/Unit --}}
                    <td class="px-6 py-4">
                        <p class="text-xs font-bold text-slate-700 leading-none mb-1.5">{{ $student->batch?->course?->name ?? '—' }}</p>
                        <p class="text-[10px] font-semibold text-slate-400">{{ $student->batch?->name ?? '' }} · Sem {{ $student->current_unit }}</p>
                    </td>

                    {{-- Attendance Stats --}}
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col items-center gap-1.5">
                            <span class="text-xs font-black text-slate-700" id="row-pct-{{ $student->id }}">
                                {{ $student->att_pct }}%
                            </span>
                            <div class="w-20 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                <div class="{{ $barColor }} h-1.5 rounded-full transition-all duration-500" id="row-bar-{{ $student->id }}" style="width:{{ min(100, $student->att_pct) }}%"></div>
                            </div>
                            <span class="text-[9px] font-bold text-slate-400 tracking-wider uppercase">{{ $student->att_present }}/{{ $student->att_total }} classes</span>
                        </div>
                    </td>

                    {{-- AI Risk Badge --}}
                    <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-3 py-1 rounded-lg text-[10px] font-bold border {{ $badgeMap[$riskLvl] }}"
                              id="row-badge-{{ $student->id }}">
                            {{ $labelMap[$riskLvl] }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 max-w-xs">
                        <p class="text-xs font-medium text-slate-600 leading-relaxed line-clamp-2"
                           id="row-remark-{{ $student->id }}">
                            {{ $pred?->ai_remark ?? 'Prediction profile pending.' }}
                        </p>
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <button class="btn-hod-predict inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-bold
                                ...
                                data-predict-url="{{ route('hod.risk.predict', $student->id) }}">
