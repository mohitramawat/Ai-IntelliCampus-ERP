...
>
    ...
    <div class="px-5 py-5 border-b border-brand-border relative flex items-center justify-between flex-shrink-0">
        <a href="{{ route(auth()->check() ? auth()->user()->dashboard_route : 'login') }}"
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto no-scrollbar">
        ...
        @foreach($menuItems as $item)
            @if(isset($item['submenu']))
                ...
                @endphp
                <div x-data="{ open: {{ $isSubActive ? 'true' : 'false' }} }" class="w-full">
                    <button @click="open = !open"
                            title="{{ $item['name'] }}"
                            class="w-full flex items-center rounded-xl transition-all duration-200 group outline-none
                                   {{ $isSubActive ? 'text-brand-accent font-semibold bg-brand-acents/30' : 'text-brand-sub hover:bg-brand-muted hover:text-brand-text' }}"
                            :class="sidebarCollapsed ? 'justify-center p-3' : 'px-3 py-2.5'">
                        
                        <span class="material-symbols-outlined text-[20px] flex-shrink-0
                                     {{ $isSubActive ? 'text-brand-accent' : 'text-brand-sub group-hover:text-brand-accent' }}"
                              :class="sidebarCollapsed ? '' : 'mr-3'">{{ $item['icon'] ?? 'circle' }}</span>
                    ...
                    <div x-show="open && !sidebarCollapsed" 
                        ...
                        @foreach($item['submenu'] as $sub)
                            @php $isActive = request()->routeIs($sub['route']) || request()->routeIs($sub['route'] . '.*'); @endphp
                            <a href="{{ route($sub['route']) }}"
                               @click="sidebarOpen = false"
                               class="flex items-center py-2 px-3 rounded-lg text-[13px] transition-all
                                      {{ $isActive ? 'text-brand-accent font-bold bg-brand-acents' : 'text-brand-sub hover:text-brand-text hover:bg-brand-muted' }}">
            ...
            @else
                ...
                <a href="{{ route($item['route']) }}"
                   ...
                   :class="sidebarCollapsed ? 'justify-center p-3' : 'px-3 py-2.5'">
                    ...
                    <span class="material-symbols-outlined text-[20px] flex-shrink-0
                                 {{ $isActive ? 'text-brand-accent' : 'text-brand-sub group-hover:text-brand-accent' }}"
                          :class="sidebarCollapsed ? '' : 'mr-3'">{{ $item['icon'] ?? 'circle' }}</span>
    ...
    <div class="p-3 border-t border-brand-border flex-shrink-0">
        ...
        <div class="flex items-center gap-3 px-2 py-2 mb-1 rounded-xl" x-show="!sidebarCollapsed" x-transition.opacity>
            ...
            <div class="min-w-0">
                <p class="text-xs font-semibold text-brand-text truncate">{{ auth()->check() ? auth()->user()->name : 'Guest' }}</p>
                <p class="text-[10px] text-brand-sub truncate">{{ auth()->check() ? ucfirst(auth()->user()->roles->first()?->name ?? '') : '' }}</p>
            </div>
        </div>
