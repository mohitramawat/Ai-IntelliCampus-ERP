...
<div class="relative bg-gradient-to-r from-brand-accent to-sky-600 rounded-2xl p-6 sm:p-8 mb-8 overflow-hidden shadow-accent">
    ...
    <div class="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="text-right hidden sm:block">
            ...
            @php $recoveryRate = $expectedFees > 0 ? round(($collectedFees / $expectedFees) * 100, 1) : 0; @endphp
            <p class="text-3xl font-extrabold text-white">{{ $recoveryRate }}%</p>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="kpi-card">
        ...
        <div>
            <div class="kpi-value text-lg sm:text-2xl">{{ $kpi['value'] }}</div>
                        ...
                        <td class="table-cell">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-brand-acents flex items-center justify-center text-brand-accent text-xs font-bold flex-shrink-0">
                                    {{ strtoupper(substr(optional(optional(optional($payment->installment)->studentUnitFee)->student->user)->name ?? '?', 0, 1)) }}
                                </div>
                                <div>
                                    <p class="text-sm font-semibold text-brand-text">{{ optional(optional(optional($payment->installment)->studentUnitFee)->student->user)->name ?? 'N/A' }}</p>
                                    <p class="text-xs text-brand-sub">{{ optional(optional(optional($payment->installment)->studentUnitFee)->student)->admission_number ?? '' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="table-cell text-sm font-medium text-brand-sub">
                            {{ optional($payment->installment)->installment_name ?? 'Installment' }}
                        </td>
                        <td class="table-cell">
                            <span class="text-sm font-bold text-brand-text">₹{{ number_format($payment->amount_paid, 2) }}</span>
                        </td>
    ...
    <div class="card flex flex-col items-center">
        ...
        <div class="relative w-44 h-44 mb-6">
            <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="40" fill="transparent" stroke="#0EA5E9" stroke-width="7"
                        stroke-dasharray="{{ $recoveryRate * 2.51 }} 251"
                        stroke-linecap="round"
                        class="transition-all duration-1000 ease-out"></circle>
            </svg>
            <div class="absolute inset-0 flex flex-col items-center justify-center">
                <span class="text-3xl font-extrabold text-brand-text">{{ $recoveryRate }}%</span>
        ...
        <div class="w-full space-y-3">
            <div class="flex justify-between items-center p-3 rounded-xl bg-brand-muted">
                ...
                <span class="text-sm font-bold text-status-success">₹{{ number_format($collectedFees, 0) }}</span>
            </div>
            <div class="flex justify-between items-center p-3 rounded-xl bg-brand-muted">
                <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-bold text-status-warning">₹{{ number_format($pendingFees, 0) }}</span>
            </div>
            <div class="flex justify-between items-center p-3 rounded-xl bg-brand-muted">
                <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-bold text-brand-text">₹{{ number_format($expectedFees, 0) }}</span>
            </div>
        </div>
    </div>
