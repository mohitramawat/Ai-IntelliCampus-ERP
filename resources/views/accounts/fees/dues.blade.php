...
<div class="card mb-8">
    <form action="{{ route('accounts.fees.dues') }}" method="GET" class="flex gap-4 items-end">
        <div class="flex-1 max-w-sm">
            ...
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="input pl-10" placeholder="Name, Roll No or Admission No...">
            </div>
        </div>
        <button type="submit" class="btn-primary h-[42px] px-6">Filter</button>
        @if(request()->filled('search'))
            <a href="{{ route('accounts.fees.dues') }}" class="btn-secondary h-[42px] px-4 flex items-center">
                    ...
                    <td class="table-cell">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-brand-muted flex items-center justify-center text-brand-sub text-xs font-bold">
                                {{ strtoupper(substr(optional($due->student->user)->name ?? '?', 0, 1)) }}
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-brand-text">{{ optional($due->student->user)->name ?? 'N/A' }}</p>
                                <p class="text-[10px] font-bold text-brand-accent uppercase">{{ optional($due->student->batch->course)->name }}</p>
                                <p class="text-[10px] text-brand-sub">{{ $due->student->roll_number }} ({{ optional($due->student->batch)->name }})</p>
                            </div>
                        </div>
                    </td>
                    <td class="table-cell text-sm font-medium text-brand-sub">₹{{ number_format($due->unit_fee, 2) }}</td>
                    <td class="table-cell text-sm font-bold text-status-success">₹{{ number_format($due->total_paid, 2) }}</td>
                    <td class="table-cell text-sm font-bold text-status-danger">₹{{ number_format($balance, 2) }}</td>
                    <td class="table-cell">
                        <div class="w-24">
                            ...
                            <div class="h-1.5 w-full bg-brand-muted rounded-full overflow-hidden">
                                <div class="h-full bg-brand-accent rounded-full transition-all duration-500" style="width: {{ $percent }}%"></div>
                            </div>
                        </div>
                    </td>
                    <td class="table-cell">
                        <button @click="selectedStudent = @json(['name' => $due->student->user->name, 'installments' => $due->installments])" class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-brand-muted hover:bg-brand-acents hover:text-brand-accent text-xs font-bold transition-all">
